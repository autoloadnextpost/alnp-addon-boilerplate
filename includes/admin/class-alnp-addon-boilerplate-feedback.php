...
if ( ! class_exists( 'ALNP_Addon_Name_Feedback' ) ) {
	class ALNP_Addon_Name_Feedback {
		 ...
		 // Get the years.
			...
			if ( $years > 1 ) {
			 /* translators: Number of years */
				return sprintf( __( '%s years', '@@name' ), $years );
			} elseif ( $years > 0 ) {
				return __( 'a year', '@@name' );
			}
			...
			if ( $weeks > 1 ) {
				...
				return sprintf( __( '%s weeks', '@@name' ), $weeks );
			} elseif ( $weeks > 0 ) {
				return __( 'a week', '@@name' );
			}
			...
			if ( $days > 1 ) {
				...
				return sprintf( __( '%s days', '@@name' ), $days );
			} elseif ( $days > 0 ) {
				return __( 'a day', '@@name' );
			}
			...
			if ( $hours > 1 ) {
				...
				return sprintf( __( '%s hours', '@@name' ), $hours );
			} elseif ( $hours > 0 ) {
				return __( 'an hour', '@@name' );
			}
			...
			if ( $minutes > 1 ) {
				...
				return sprintf( __( '%s minutes', '@@name' ), $minutes );
			} elseif ( $minutes > 0 ) {
				return __( 'a minute', '@@name' );
			}
			...
			if ( $seconds > 1 ) {
				...
				return sprintf( __( '%s seconds', '@@name' ), $seconds );
			} elseif ( $seconds > 0 ) {
				return __( 'a second', '@@name' );
			}
		}
			...
			<div class="notice updated alnp-feedback-notice">
				<div class="alnp-feedback-notice-inner">
					<div class="alnp-feedback-notice-icon">
						...
						<img src="https://ps.w.org/@@name/assets/icon-256x256.png" alt="<?php printf( esc_attr__( '%1$s WordPress Plugin', '@@name' ), esc_attr( $this->name ) ); ?>" />
					</div>
					<div class="alnp-feedback-notice-content">
						<?php /* translators: 1. Name */ ?>
						<h3><?php printf( esc_html__( 'Are you enjoying %1$s?', '@@name' ), esc_html( $this->name ) ); ?></h3>
						<p>
							<?php /* translators: 1. Name, 2. Time */ ?>
							<?php printf( esc_html__( 'You have been using %1$s for %2$s now! Mind leaving a quick review to let me know know what you think? I\'d really appreciate it!', '@@name' ), esc_html( $this->name ), esc_html( $time ) ); ?>
						</p>
					</div>
					<div class="alnp-install-now">
						<?php printf( '<a href="%1$s" class="button button-primary alnp-install-button" target="_blank">%2$s</a>', esc_url( ALNP_ADDON_REVIEW_URL . '#new-post' ), esc_html__( 'Leave a Review', '@@name' ) ); ?>
						<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No thanks / I already have', '@@name' ); ?></a>
					</div>
				</div>
			</div>
			<?php
...
/*
 * Instantiate the ALNP_Addon_Name_Feedback class.
 */
new ALNP_Addon_Name_Feedback(
	array(
